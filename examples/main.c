...
Change this value to the value that the application requires. */
      ...
      // result_data[idx1 + result->size[1] * idx0] = argInit_real_T();
    }
  }
  return result;
}
...
Change this value to the value that the application requires. */
  ...
  // 导入txt文件数据
  FILE *file0;
  file0 = fopen("..\\datasets\\fit_data4.txt", "r");
  ...
  char buffer[1024];  // 用于存储每行数据的缓冲区
  int totalLines = 0;                 // 总行数计数器
  while(fgets(buffer, sizeof(buffer), file0)!= NULL) {
    totalLines++;
  }
  fclose(file0);
  printf("totalLines:%d\n", totalLines);
  ...
  while(fgets(line, sizeof(line), file)) {
    ...
    while (token != NULL && column_count < NUM_COLUMNS) {
      ...
      if (column_count < 3) {
        ...
        result.pe->data[row_count + totalLines * column_count] = cur_value;                    // 按列排列
        // printf("STEP2: %.6f\n", result.pe->data[column_count + row_count * 3]);
        // printf("STEP1: %f\n", result.pe->data[row_count + totalLines * column_count]);

      } else if (column_count < 6) {
        // result.marker_location->data[column_count - 3 + row_count * 3] =  cur_value;  // 按行排列
        result.marker_location->data[row_count + totalLines *(column_count - 3)] =  cur_value;  // 按列排列
        // printf("STEP3: %.6f\n", result.marker_location->data[column_count - 3 + row_count * 3]);

      } else {
        int idx0 = (column_count - 6) / 3;  // 商
        int idx1 = (column_count - 6) % 3;  // 余数
        // result.R_b2e->data[(idx1 + 3 * idx0) + 9 * row_count] = cur_value;          // 按行排列
        result.R_b2e->data[(idx0 + 3 * idx1) + 9 * row_count] = cur_value;          // 按列排列
...
void main_PSO_marker_extric_estimate(void)
{
  ...
  /*  打印最优结果 */
  printf("STEP10:bestPosition[0]= %f\n", bestPosition[0]);
  printf("STEP10:bestPosition[1]= %f\n", bestPosition[1]);
  printf("STEP10:best_fit_error = %f\n", bestValue);
}
